...
{
    /// <summary>Elenca (paginato) le prenotazioni del tenant filtrate per data/staff/servizio/stato, ordinate per
    /// data e ora. <paramref name="page"/> è 1-based; <paramref name="pageSize"/> viene limitato lato servizio.</summary>
    Task<Result<PagedResponse<AdminBookingResponse>>> ListAsync(
