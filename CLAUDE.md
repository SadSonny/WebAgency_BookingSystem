...
**Fase attuale:** V1 + **V2 email** + **hardening PH-1..PH-5** + **V2.1 salone reale** + **V2.2 hardening perf/sicurezza** + **V2.3 onboarding Owner** (2026-06-16) + **Agency Provisioning API** (2026-06-18) + **Monitor OPS 4.2** (2026-06-18) + **GDPR DSAR + consenso arricchito 4.3** (2026-06-19). **126 unit test verdi** (suite unit; integration non rieseguita in questa sessione).

**GDPR 4.3 — DSAR on-demand + consenso (2026-06-19):** endpoint admin `GET/POST /api/v1/admin/gdpr/customer[/erase]` per export (diritto d'accesso, sempre 200) e cancellazione (anonimizza prenotazioni + elimina outbox del cliente, atomico) per email, tenant-scoped, con audit PII-free (`subjectRef` HMAC). Aggiunto `Booking.GdprConsentVersion` (versione informativa, inviata dal client). 1 migration (`AddGdprConsentVersion`). Doc `Claude_Instructions/GDPR_COMPLIANCE.md`. Erasure = anonimizzazione (riusa marker `DataRetentionService`).
...
# DSAR GDPR (V4.3, JWT, tenant-scoped)
GET    /api/v1/admin/gdpr/customer?email=          → export dati cliente (diritto d'accesso; sempre 200)
POST   /api/v1/admin/gdpr/customer/erase           body: { email }  → anonimizza prenotazioni + elimina outbox (404 se nulla, 422 se email invalida)
```
