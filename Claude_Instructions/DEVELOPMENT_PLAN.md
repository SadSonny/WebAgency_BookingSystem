...
## Stato: MONITOR OPS 4.2 COMPLETATO (PARZIALE) (2026-06-18)

> **V1 + V2 email + PH-1..PH-5 + V2.1 salone reale + V2.2 hardening + V2.3 onboarding Owner + Agency Provisioning API + Monitor OPS 4.2.** Build verde
> (0 warning), **114 unit test verdi**. Monitor OPS (2026-06-18): `OpsAlertMonitorJob`, `OpsAlertScanner`, `DbLogErrorSource`, `DbHealthProbe`,
> `LogOnlyAlertChannel`, `TelegramAlertChannel`, `AddOpsAlerting` DI, config `Ops:Alerting`, 17 nuovi unit test.
> **→ Prossimo:** alert `OutboxEmail.Status == Failed` (4.2 restante). Poi Railway deploy (applicare TUTTE le migration incl. `AddPlatformAdmin`; impostare `PLATFORM_SETUP_TOKEN`;
...
| 2026-06-18 | OPS 4.2 | **Monitor OPS — BackgroundService + DI + config**: `OpsAlertMonitorJob` (BackgroundService, scheduling-only); `OpsAlertScanner` (singleton, watermark/flag in-memory, RunOnceAsync); `DbLogErrorSource` (lettura `logs` via SQL grezzo, scope per chiamata); `DbHealthProbe` (connettività DB); `LogOnlyAlertChannel` (riga `[OPS-ALERT]` su console/DB); `TelegramAlertChannel` (Bot API). Canale scelto da `OpsAlertOptions.FromConfiguration` (fallback LogOnly se credenziali Telegram mancanti). `AddOpsAlerting` in `DependencyInjection.cs` (no-op se `Enabled=false`). Config `Ops:Alerting` in `appsettings.json`. Env: `OPS_ALERT_CHANNEL`, `OPS_ALERT_TELEGRAM_BOT_TOKEN`, `OPS_ALERT_TELEGRAM_CHAT_ID`. Build 0 warning, **114 unit test verdi** (+17 nuovi: scanner, canali, sorgente, options). Next: alert `OutboxEmail.Status == Failed`. |
